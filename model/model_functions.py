...
doest work parmaeters ??   
def VIEW_INDEPENDENTxVIEW_DEPENDENT(alpha, beta, lamd_a, lamd_a_d, VPN_output, new_ID, numb_prev_presentations, stim_IDs, stim_IDs_perspective, verbose):
    ...
    for stim_ID_depend, stim_ID_indipend, action,num_pres_depend, num_pres_independ,trial in zip(stim_IDs_perspective,stim_IDs,VPN_output,numb_presentations,numb_prev_presentations,range(len(stim_IDs))):
        ...
        new_Vfam = old_fam_indipend + (alpha * (lamd_a - old_fam_indipend))
        # update view-dependent
        new_fam_dep = old_fam_depend + (alpha * (lamd_a-old_fam_depend)) # compute new stim familiarity
        ...
        ### get totfam @ time t
        totfam = old_fam_indipend + new_fam_dep   
        history_total.append(totfam)
